...
let tasks = {
  pending: {
    ...
    items: [
      ...
      {
        id: fetchID(),
        title: "Implement DND on the task page",
        comments: [],
      },
    ],
  },
  ...
  ongoing: {
    ...
    items: [
      ...
      {
        id: fetchID(),
        title: "Design DND on the task page",
        comments: [],
      },
    ],
  },
...
// setting up socket.io for real time connection
socketIO.on("connection", (socket) => {
  ...
  socket.on("task:deleteTask", (data) => {
    const { category, id } = data;
    const taskItems = tasks[category].items;
    tasks[category].items = taskItems.filter((item) => item.id !== id);
    socket.emit("task:update", tasks);
  });
...
app.get("/api/tasks/:category/:id", (req, res) => {
